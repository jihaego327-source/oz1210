...
> **참고**: JWT 템플릿은 2025년 4월 1일부터 deprecated되었으며, 네이티브 통합이 권장됩니다.
...
1. **인증 흐름**:
   ...
   - Clerk가 사용자 인증 처리
   - Clerk Dashboard에서 Supabase 통합 활성화 (자동으로 `"role": "authenticated"` JWT claim 추가)
   - Supabase에서 Clerk를 third-party auth provider로 설정
   - `SyncUserProvider`가 로그인 시 자동으로 Clerk 사용자를 Supabase `users` 테이블에 동기화
   - Supabase 클라이언트가 Clerk 세션 토큰을 사용하여 인증 (JWT 템플릿 불필요)
...
2. **Supabase 클라이언트 파일들** (`lib/supabase/`):
   ...
   - `server.ts`: Server Component/Server Action용 (createClient)
     ...
     - Next.js 15 패턴: `await createClient()` 사용
     - 공식 문서 예제와 동일한 패턴: `import { createClient } from '@/lib/supabase/server'`
...
3. **사용자 동기화**:
   ...
   - `app/api/sync-user/route.ts`: 실제 동기화 로직 (API 라우트, service-role 클라이언트 사용)
...
- `lib/supabase/clerk-client.ts`: Client Component용 Supabase 클라이언트 (Clerk 통합)
- `lib/supabase/server.ts`: Server Component/Server Action용 Supabase 클라이언트 (Clerk 통합)
- `lib/supabase/service-role.ts`: 관리자 권한용 Supabase 클라이언트 (RLS 우회)
- `lib/supabase/client.ts`: 공개 데이터용 Supabase 클라이언트 (인증 불필요)
- `components.json`: shadcn/ui 설정

## Clerk + Supabase 통합 참고 자료
