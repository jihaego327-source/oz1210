...
- 환경별 Supabase 클라이언트 분리:
  ...
  - Server Component용 (`createClient` - 공식 문서 패턴)
...
2. 설정 옵션을 선택한 후 **"Activate Supabase integration"** 클릭
3. 표시되는 **Clerk domain**을 복사하여 저장 (예: `https://your-app-12.clerk.accounts.dev`)
   - 이 값은 다음 단계에서 사용됩니다
...
4. **"Add provider"** 클릭하고 목록에서 **"Clerk"** 선택
5. 3-1에서 복사한 **Clerk domain**을 붙여넣기
6. **"Save"** 또는 **"Add Provider"** 클릭

> **참고**: 이 통합은 Clerk 세션 토큰에 `"role": "authenticated"` JWT claim을 자동으로 추가하여 Supabase API 요청 시 인증된 사용자로 인식되도록 합니다.

**3-3. 통합 확인**

- [Clerk 공식 Supabase 통합 가이드](https://clerk.com/docs/guides/development/integrations/databases/supabase)에서 상세 정보 확인
- [Supabase Third-Party Auth 문서](https://supabase.com/docs/guides/auth/third-party/overview) 참고
...
> **⚠️ 주의**: 
> - `service_role` 키는 모든 RLS를 우회하는 관리자 권한이므로 절대 공개하지 마세요!
> - `NEXT_PUBLIC_SUPABASE_ANON_KEY`는 Supabase Dashboard의 "anon public" 키입니다 (공식 문서의 `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`와 동일)
...
**테스트 페이지:**
- `/instruments`: Supabase 공식 Next.js 퀵스타트 예제 (데이터 조회 테스트)
...
> **참고**: 
> - 로컬라이제이션 기능은 현재 실험적(experimental) 기능입니다.
> - Clerk Account Portal은 여전히 영어로 표시됩니다.
> - 커스텀 에러 메시지나 특정 텍스트를 변경하려면 [Clerk 로컬라이제이션 가이드](https://clerk.com/docs/guides/customizing-clerk/localization)를 참고하세요.
...
│   ├── instruments/       # Supabase 공식 퀵스타트 예제
