...
- [x] 페이지네이션
  ...
  - [x] 페이지 번호 선택 방식
    ...
    - [x] 페이지 번호 버튼 구현 (현재 페이지 하이라이트)
    - [x] 이전/다음 페이지 버튼
    - [x] 페이지 범위 표시 (1 ... 5 6 7 ... 20)
    - [x] 페이지 정보 표시 (1-20 / 전체 1,234개)
    - [x] 반응형 디자인 (모바일 간소화)
    - [x] URL 쿼리 파라미터 동기화
    - [x] TourMapLayout에 pagination prop 추가
    - [x] app/page.tsx에서 pagination 정보 전달
  ---
  에러 사항 해결
...
- [x] 최종 통합 및 스타일링
  ...
  - [ ] 반응형 디자인 확인 (모바일/태블릿/데스크톱) (수동 확인 필요)
  - [x] 로딩 상태 개선
    - [x] 에러 시 재시도 기능 개선 (TourList에서 자동 처리)
  - [x] 에러 처리 개선
    - [x] 에러 메시지 개선 (더 구체적인 메시지)
    - [x] 에러 로깅 추가 (console.error)
    - [x] 재시도 기능 자동 처리 (페이지 새로고침)
  ---
  추가 개발 사항
  - [x] 페이지네이션 컴포넌트 생성 및 통합
  - [x] 페이지네이션 정보 표시 (1-20 / 전체 1,234개)
  - [x] 모바일 반응형 페이지네이션 (간소화된 버튼)
  - [x] 접근성 개선 (ARIA 라벨, aria-current)
...
- [x] 통계 데이터 수집
  - [x] `lib/api/stats-api.ts` 생성
    ...
    ---
    추가 개발 사항
    - [x] Next.js `unstable_cache`를 사용한 캐싱 구현 (revalidate: 3600, tags 사용)
    - [x] 내부 함수와 공개 함수 분리 (캐싱 적용을 위한 구조)
    - [x] 지역별 통계: 각 지역별로 모든 타입을 합산하여 전체 개수 계산
